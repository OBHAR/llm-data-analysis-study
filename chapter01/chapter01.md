...
1. 월별 completed 주문 금액을 비교한다.
2. 주문 수와 주문당 평균 금액을 분리해 변화 여부를 확인한다.
3. 상품 카테고리별 금액·수량을 비교한다.
4. 고객별 구매 금액 분포를 확인한다.
5. 주문 상태별 건수과 비율을 비교한다.

### 결과 관찰

LLM은 시간별 변화, 주문 단위 지표, 카테고리 비교처럼 현재 파일 구조와 연결 가능한 질문을 제안했다. 반면 컬럼 존재 여부와 정확한 계산 범위는 답변만으로 확정할 수 없었다.

### 나의 해석과 판단

월별 주문 수·객단가·카테고리를 함께 보자는 제안은 선택한 목적에 적합했다. 다만 분석 기간, 매출 계산식, `completed` 필터는 사람이 실제 데이터에 맞춰 명시해야 했다.
...
LLM 답변은 실제 컬럼 타입·결측·중복을 알지 못한다. 따라서 제안은 초안으로만 사용하고 데이터 확인 뒤 반영해야 한다.
